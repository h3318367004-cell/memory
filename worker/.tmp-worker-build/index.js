...
async function dream(env, input = {}) {
  ...
  const summary = await remember(env, {
    externalId: `dream:${(/* @__PURE__ */ new Date()).toISOString()}`,
    kind: "summary",
    layer: "dream",
    title: `Dream ${clean(input.kind) || "ad_hoc"} ${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}`,
    text,
...
function normalizeMemory(input = {}) {
  ...
  return {
    ...
    memory_date: clean(input.memoryDate || input.memory_date) || (/* @__PURE__ */ new Date()).toISOString(),
...
__name(normalizeMemory, "normalizeMemory");
function normalizeMemoryPatch(input = {}) {
  ...
  for (const [key, value] of Object.entries(patch)) {
    if (value === void 0) delete patch[key];
  }
...
__name(normalizeMemoryPatch, "normalizeMemoryPatch");
...
__name(buildDreamText, "buildDreamText");
